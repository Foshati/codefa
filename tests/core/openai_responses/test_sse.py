...
from codefa.core.anthropic.stream_contracts import parse_sse_text
from codefa.core.anthropic.streaming import format_sse_event
from codefa.core.async_iterators import AsyncCloseable
from codefa.core.failures import ExecutionFailure, FailureKind
from codefa.core.openai_responses import (
    OpenAIResponsesAdapter,
    OpenAIResponsesRequest,
)
from codefa.core.openai_responses.anthropic_sse import (
    AnthropicSseEvent,
    iter_sse_events,
)
...
async def test_responses_transform_closes_direct_event_source_on_early_close() -> None:
    ...
    with patch(
        "codefa.core.openai_responses.stream.iter_sse_events",
        return_value=events,
    ):
...
async def test_anthropic_tool_stream_converts_to_function_call_item() -> None:
    ...
    assert function_call["arguments"] == '{"value":"CODEFA"}'


@pytest.mark.asyncio
async def test_anthropic_function_tool_arguments_are_normalized() -> None:
    response = await _completed_response_from_sse(
        _aiter(_anthropic_tool_stream(partial_json='{ "value" : "CODEFA" }')),
        {"model": "nvidia_nim/test-model", "stream": True},
    )

    assert response["output"][0]["arguments"] == '{"value":"CODEFA"}'
...
async def test_anthropic_malformed_function_tool_arguments_fail_response() -> None:
    text = await _collect_sse(
        _responses_sse(
            _aiter(_anthropic_tool_stream(partial_json='{"value":"CODEFA" "bad"}')),
            {"model": "nvidia_nim/test-model", "stream": True},
        )
    )
...
async def test_overlapping_text_and_tool_blocks_keep_reserved_output_indexes() -> None:
    ...
    assert completed["output"][1]["arguments"] == '{"value":"CODEFA"}'
