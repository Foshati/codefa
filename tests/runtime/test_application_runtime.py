...
import codefa.messaging.session.persistence as persistence_module
from codefa.config.admin.persistence import PreparedAdminUpdate
from codefa.config.settings import Settings
from codefa.messaging.command_context import StopOutcome
from codefa.messaging.platforms.ports import (
    InboundMessageHandler,
    MessagingPlatformComponents,
    MessagingStartupNotice,
)
from codefa.messaging.session import SessionStore
from codefa.messaging.workflow import MessagingWorkflow
from codefa.providers.runtime import ProviderRuntime
from codefa.runtime.application import ApplicationRuntime
from codefa.runtime.provider_manager import ProviderRuntimeManager
...
async def test_provider_apply_constructs_before_commit_then_publishes(tmp_path) -> None:
    ...
    with (
        patch(
            "codefa.runtime.application.prepare_admin_update",
            return_value=prepared,
        ),
        patch(
            "codefa.runtime.application.commit_prepared_admin_update",
            side_effect=commit,
        ),
    ):
...
async def test_candidate_failure_never_commits_and_preserves_current(tmp_path) -> None:
    ...
    with (
        ...
        patch("codefa.runtime.application.commit_prepared_admin_update") as commit,
...
async def test_public_start_retries_transient_partial_messaging_cleanup() -> None:
    ...
    with (
        ...
        patch(
            "codefa.runtime.application.messaging_platform_factory.create_messaging_components",
...
async def test_composition_records_runtime_before_workspace_setup() -> None:
    ...
    with (
        patch(
            "codefa.runtime.application.os.makedirs",
...
async def test_composition_publishes_startup_notice_after_runtime_and_repair() -> None:
    ...
    with (
        patch(
            "codefa.runtime.application.cli_managed.ManagedClaudeSessionManager",
            return_value=cli_manager,
        ) as manager_constructor,
        patch("codefa.runtime.application.messaging_session.SessionStore"),
        patch(
            "codefa.runtime.application.messaging_workflow_module.MessagingWorkflow",
