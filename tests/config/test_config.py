...
from codefa.config.constants import (
...
from codefa.config.model_refs import (
...
class TestSettings:
    ...
    def test_open_admin_browser_loads_from_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEFA_OPEN_BROWSER", "false")
        monkeypatch.setitem(Settings.model_config, "env_file", ())

        assert Settings().open_admin_browser is False

    def test_default_claude_workspace_uses_codefa_home(self, monkeypatch, tmp_path):
        ...
        assert messaging_state_dir_path() == tmp_path / ".codefa" / "agent_workspace"
        assert not hasattr(settings, "claude_workspace")

    def test_server_log_path_uses_codefa_home(self, monkeypatch, tmp_path):
        """The server log location is fixed under ~/.codefa."""
        from codefa.config.paths import server_log_path

        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("USERPROFILE", str(tmp_path))

        assert server_log_path() == tmp_path / ".codefa" / "logs" / "server.log"
    ...
    def test_blank_claude_workspace_uses_codefa_home(self, monkeypatch, tmp_path):
    ...
    def test_base_url_constant(self):
        ...
        from codefa.config.provider_catalog import NVIDIA_NIM_DEFAULT_BASE
    ...
    def test_route_reasoning_inherits_root_policy(self, monkeypatch):
        ...
        from codefa.application.routing import ModelRouter
        from codefa.config.settings import Settings
...
class TestPerModelMapping:
    ...
    def test_configured_chat_model_refs_collects_unique_models_with_sources(
        ...
        monkeypatch.setenv("CODEFA_SMOKE_MODEL_NVIDIA_NIM", "nvidia_nim/smoke")
