...
from codefa.application.errors import (
    ApplicationUnavailableError,
    InvalidRequestError,
)
from codefa.config.settings import Settings
from codefa.messaging.transcription import TranscriptionService
from codefa.providers.nvidia_nim.client import NvidiaNimProvider
from codefa.providers.nvidia_nim.voice import NvidiaNimTranscriber
from codefa.runtime.application import (
    ApplicationRuntime,
    startup_failure_message,
    warn_if_process_auth_token,
)
from codefa.runtime.asgi import RuntimeASGIApp
from codefa.runtime.bootstrap import _create_transcriber, build_asgi_app
...
def _redirect_codefa_home(monkeypatch, tmp_path):
    home = tmp_path / "home"
...
def test_warn_if_process_auth_token_logs_warning(monkeypatch):
    ...
    with patch("codefa.runtime.application.logger.warning") as warning:
...
def test_application_error_handler_does_not_log_error_message():
    ...
    with patch("codefa.api.app.logger.error") as log_error:
...
def test_bootstrap_configures_default_log_and_publishes_only_services(tmp_path):
    ...
    with (
        patch(
            "codefa.runtime.bootstrap.server_log_path",
            return_value=log_path,
        ),
        patch("codefa.runtime.bootstrap.configure_logging") as configure,
    ):
...
def test_bootstrap_honors_process_log_file_override(monkeypatch, tmp_path):
    ...
    with patch("codefa.runtime.bootstrap.configure_logging") as configure:
...
async def test_bootstrap_constructs_isolated_runtime_resource_graphs() -> None:
    ...
    with patch("codefa.runtime.bootstrap.configure_logging"):
