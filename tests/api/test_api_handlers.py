...
from codefa.api.handlers import (
    MessagesHandler,
    ResponsesHandler,
    TokenCountHandler,
)
...
from codefa.core.anthropic.models import (
    Message,
    MessagesRequest,
    TokenCountRequest,
)
...
from codefa.core.openai_responses import OpenAIResponsesRequest
...
async def test_messages_handler_forces_no_thinking_for_safety_classifier() -> None:
    ...
    with patch("codefa.api.handlers.messages.trace_event") as trace_mock:
        response = await handler.create(request)
        assert isinstance(response, StreamingResponse)
    ...
    assert _trace_events(
        trace_mock, "codefa.api.optimization.safety_classifier_no_thinking"
    ) == [
        {
            "stage": "routing",
...
async def test_messages_handler_preserves_thinking_for_non_classifier() -> None:
    ...
    assert (
        _trace_events(
            trace_mock,
            "codefa.api.optimization.safety_classifier_no_thinking",
        )
        == []
    )
...
def test_token_count_handler_routes_and_counts_tokens() -> None:
    ...
    with patch("codefa.api.handlers.token_count.trace_event") as trace:
