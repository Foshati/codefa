...
from codefa.application.model_metadata import (
    ProviderModelInfo,
    ProviderModelRefreshResult,
)
from codefa.config.admin.values import MASKED_SECRET
...
def test_admin_unexpected_errors_are_never_cached(monkeypatch, tmp_path):
    ...
    with patch(
        "codefa.api.admin_routes.load_config_response",
...
def test_admin_api_fetches_bypass_browser_cache():
    script = Path("src/codefa/api/admin_static/admin.js").read_text(encoding="utf-8")
...
def test_admin_static_model_combobox_owns_dropdown_and_search_behavior():
    script = Path("src/codefa/api/admin_static/admin.js").read_text(encoding="utf-8")
    styles = Path("src/codefa/api/admin_static/admin.css").read_text(encoding="utf-8")
...
def test_admin_config_masks_secrets_and_exposes_manifest(monkeypatch, tmp_path):
    ...
    assert "CODEFA_OPEN_BROWSER" in keys
    ...
    open_browser_field = next(
        field for field in body["fields"] if field["key"] == "CODEFA_OPEN_BROWSER"
    )
...
def test_admin_apply_persists_open_browser_for_next_launch(monkeypatch, tmp_path):
    ...
    response = _local_client(app).post(
        ...
        json={"values": {"CODEFA_OPEN_BROWSER": False}},
    )
    ...
    assert "CODEFA_OPEN_BROWSER=false" in managed_env.read_text(encoding="utf-8")
...
def test_admin_apply_preserves_hidden_diagnostics_and_smoke_values(
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "CODEFA_SMOKE_MODEL_ZAI=zai/smoke-model",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    assert "CODEFA_SMOKE_MODEL_ZAI=zai/smoke-model" in text
...
def test_admin_first_apply_migrates_repo_env(monkeypatch, tmp_path):
    ...
    assert response.status_code == 200
    managed_text = (tmp_path / ".codefa" / ".env").read_text("utf-8")
...
def test_admin_local_provider_status_reports_reachable(monkeypatch, tmp_path):
    ...
    with patch("codefa.api.admin_routes.httpx.AsyncClient", FakeAsyncClient):
