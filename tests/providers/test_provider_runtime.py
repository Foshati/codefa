...
from codefa.application.errors import UnknownProviderError
from codefa.config.nim import NimSettings
...
from codefa.providers.cloudflare import CloudflareProvider
from codefa.providers.deepseek import DeepSeekProvider
from codefa.providers.gemini import GeminiProvider
from codefa.providers.github_models import GitHubModelsProvider
from codefa.providers.lmstudio import LMStudioProvider
from codefa.providers.mistral import MistralProvider
from codefa.providers.nvidia_nim import NvidiaNimProvider
from codefa.providers.open_router import OpenRouterProvider
from codefa.providers.openai_chat import (
    OPENAI_CHAT_PROFILES,
    OpenAIChatProvider,
)
from codefa.providers.runtime import (
    ProviderRuntime,
    build_provider_config,
    create_provider,
)
from codefa.providers.vertex import VertexProvider
...
def test_importing_runtime_does_not_eager_load_other_adapters() -> None:
    ...
    code = (
        "import sys\n"
        "import codefa.providers.runtime\n"
        "assert 'codefa.providers.open_router' not in sys.modules\n"
    )
    proc = subprocess.run(
...
def test_local_provider_factory_resolves_catalog_static_credential(
    ...
    config = build_provider_config(descriptor, settings)
    with patch("codefa.providers.openai_chat.provider.AsyncOpenAI"):
...
def test_create_provider_instantiates_each_builtin():
    ...
    with (
        patch("codefa.providers.openai_chat.provider.AsyncOpenAI"),
        patch("httpx.AsyncClient"),
        patch(
            "codefa.providers.runtime.factory.ProviderAdmissionController",
