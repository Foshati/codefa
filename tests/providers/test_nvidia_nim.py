...
from codefa.core.failures import ExecutionFailure
from codefa.core.reasoning import ReasoningEffort, ReasoningPolicy
from codefa.providers.admission import UPSTREAM_TRANSIENT_TOTAL_ATTEMPTS
...
async def test_init(provider_config):
    ...
    with patch("codefa.providers.openai_chat.provider.AsyncOpenAI") as mock_openai:
    ...
    req = make_request(
        messages=[
            ...
            message(
                ...
                [
                    block(
                        type="tool_result", tool_use_id=tool_id, content="CODEFA_206"
                    ),
...
async def test_stream_response_restores_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        name="Grep",
        arguments=json.dumps({"pattern": "needle", "-A": 2, "_codefa_arg_type": "py"}),
    )
    ...
    assert "_codefa_arg_A" not in properties
    assert "_codefa_arg_type" in properties

    deltas = _input_json_deltas(events)
    assert len(deltas) == 1
    assert json.loads(deltas[0]) == {"pattern": "needle", "-A": 2, "type": "py"}
    assert "_codefa_arg_type" not in deltas[0]
...
async def test_stream_response_restores_nested_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        ...
        arguments=json.dumps(
            {"parent": {"_codefa_arg_type": "page_id", "id": "page_123"}}
        ),
    )
