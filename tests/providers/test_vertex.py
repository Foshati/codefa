...
from codefa.config.provider_catalog import VERTEX_AI_API_ROOT
...
from codefa.providers.vertex.auth import GoogleAccessTokenProvider
from codefa.providers.vertex.endpoint import (
    vertex_openai_base_url,
    vertex_publisher_models_url,
    vertex_service_endpoint,
)
from codefa.providers.vertex.models import extract_vertex_model_page
...
def test_vertex_provider_supplies_renewable_token_callback_to_openai() -> None:
    ...
    with (
        patch("codefa.providers.openai_chat.provider.AsyncOpenAI") as openai_client,
        patch("codefa.providers.vertex.client.httpx.AsyncClient"),
    ):
...
def test_vertex_rejects_caller_thinking_config_with_codefa_reasoning_control() -> None:
    provider = _provider()
    request = make_messages_request(
        "google/gemini",
        thinking=None,
