...
echo "uv:$*" >> "$CALL_LOG"
if [ "${{1:-}}" = "--version" ]; then
    if [ "${{CODEFA_RUNNING_PHASE:-}}" = "late" ]; then
        : > "$CODEFA_PROCESS_MARKER"
    fi
...
if [ "${{1:-}}" = "tool" ] && [ "${{2:-}}" = "install" ]; then
    if [ "$FAIL_STEP" = "codefa-install" ]; then
        exit 33
    fi
...
""",
        ...
        self.env["CODEFA_PS_OUTPUT"] = process_line
...
def posix_harness(tmp_path: Path) -> PosixHarness:
    ...
    _write_executable(
        bin_dir / "pgrep",
        """#!/bin/sh
[ -n "${CODEFA_RUNNING_COMMAND:-}" ] || exit 1
if [ "${CODEFA_RUNNING_PHASE:-early}" = "late" ] && [ ! -e "$CODEFA_PROCESS_MARKER" ]; then
    exit 1
fi
case "$*" in
    *"$CODEFA_RUNNING_COMMAND"*) printf '4242\n'; exit 0 ;;
    *) exit 1 ;;
esac
""",
    )
...
name=${0##*/}
echo "$name:$*" >> "$CALL_LOG"
if [ "$FAIL_STEP" = "codefa-verify" ]; then
    exit 36
fi
if [ "$name" = "codefa-server" ] && [ "${1:-}" = "--version" ]; then
    echo "codefa 3.5.18"
fi
""",
    )
    ...
    env.update(
        {
            ...
            "CODEFA_PROCESS_MARKER": str(tmp_path / "codefa-process-ready"),
            "CODEFA_RUNNING_COMMAND": "",
            "CODEFA_RUNNING_PHASE": "early",
            "FAIL_STEP": "",
        }
    )
...
def test_install_sh_fresh_install_is_verified(posix_harness: PosixHarness) -> None:
    ...
    assert "codefa ready!" in result.stdout
    ...
    assert any(
        call.startswith(
            "uv:tool install --force --refresh-package codefa "
            "--python 3.14.0 codefa @ "
            "https://github.com/Foshati/codefa/archive/refs/heads/main.zip"
        )
        for call in calls
    )
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    assert "codefa ready!" not in result.stdout
    forbidden = {
        ...
        "codefa-install": "uv:tool update-shell",
        "path-update": "uv:tool dir --bin",
        "codefa-missing": "codefa-server:--version",
    }.get(failure)
...
def test_install_sh_voice_flags_only_change_codefa_spec(
    posix_harness: PosixHarness,
) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert any(
        "--torch-backend cu130 codefa[voice,voice_local] @ "
        "https://github.com/Foshati/codefa/archive/refs/heads/main.zip" in call
...
@pytest.mark.parametrize("command_name", CODEFA_COMMANDS)
def test_install_sh_rejects_running_codefa_before_mutation(
    posix_harness: PosixHarness,
    command_name: str,
) -> None:
    posix_harness.env["CODEFA_RUNNING_COMMAND"] = command_name
...
def test_install_sh_rechecks_for_codefa_process_before_tool_replacement(
    posix_harness: PosixHarness,
) -> None:
    posix_harness.add_client("claude")
    posix_harness.add_client("codex")
    ...
    assert result.returncode != 0
    assert "codefa-server (PID 4242)" in result.stderr
...
def test_install_sh_ignores_similarly_named_process(
    ...
    posix_harness.env["CODEFA_RUNNING_COMMAND"] = "codefa-server-helper"
...
@pytest.mark.parametrize(
    ...
    (
        ("codefa", "/home/user/.local/bin/codefa"),
        ("codefa-server", "/usr/bin/python3 /home/user/.local/bin/codefa-server"),
    ),
)
...
exit /b 59
:version
if "%CODEFA_RUNNING_PHASE%"=="late" type nul > "%CODEFA_PROCESS_MARKER%"
if "%FAIL_STEP%"=="uv-verify" exit /b 52
echo uv {version}
exit /b 0
:install
if "%FAIL_STEP%"=="codefa-install" exit /b 53
...
def powershell_harness(
    ...
    (fixtures / "codefa-command.cmd").write_text(
        """@echo off
for %%I in ("%~f0") do set "CODEFA_NAME=%%~nI"
echo %CODEFA_NAME%:%*>>"%CALL_LOG%"
if "%FAIL_STEP%"=="codefa-verify" exit /b 55
if "%CODEFA_NAME%"=="codefa-server" if "%1"=="--version" echo codefa 3.5.18
exit /b 0
""",
        encoding="utf-8",
    )
...
function Get-Process {
    ...
    if ([string]::IsNullOrWhiteSpace($env:CODEFA_RUNNING_COMMAND)) {
        return
    }
    if (
        $env:CODEFA_RUNNING_PHASE -eq "late" -and
        -not (Test-Path -LiteralPath $env:CODEFA_PROCESS_MARKER)
    ) {
        return
    }
    foreach ($requestedName in $Name) {
        if ($requestedName -eq $env:CODEFA_RUNNING_COMMAND) {
            [pscustomobject] @{ Id = 4242; ProcessName = $requestedName }
        }
    }
}
$installer = [scriptblock]::Create([IO.File]::ReadAllText($env:CODEFA_INSTALLER))
& $installer @args
""",
        encoding="utf-8",
    )
    ...
    env.update(
        {
            ...
            "FAKE_FIXTURES": str(fixtures),
            "FAKE_TOOL_BIN": str(tool_bin),
            "CODEFA_INSTALLER": str(_repo_root() / "scripts" / "install.ps1"),
            "CODEFA_PROCESS_MARKER": str(tmp_path / "codefa-process-ready"),
            "CODEFA_RUNNING_COMMAND": "",
            "CODEFA_RUNNING_PHASE": "early",
            "FAIL_STEP": "",
...
def test_install_ps1_voice_flags_only_change_codefa_spec(
    powershell_harness: PowerShellHarness,
) -> None:
...
@pytest.mark.parametrize("command_name", CODEFA_COMMANDS)
def test_install_ps1_rejects_running_codefa_before_mutation(
    powershell_harness: PowerShellHarness,
    command_name: str,
) -> None:
    powershell_harness.env["CODEFA_RUNNING_COMMAND"] = command_name
...
def test_install_ps1_rechecks_for_codefa_process_before_tool_replacement(
    powershell_harness: PowerShellHarness,
) -> None:
    powershell_harness.add_client("claude")
    powershell_harness.add_client("codex")
...
def test_install_ps1_ignores_similarly_named_process(
    ...
    powershell_harness.env["CODEFA_RUNNING_COMMAND"] = "codefa-server-helper"
...
def test_installers_use_native_clients_and_single_python_selection() -> None:
    ...
    for text in (shell, powershell):
        for command_name in CODEFA_COMMANDS:
        ...
        assert "https://github.com/Foshati/codefa/archive/refs/heads/main.zip" in text
