...
def posix_uninstall_harness(tmp_path: Path) -> PosixUninstallHarness:
    ...
    codefa_home = home / ".codefa"
    log = tmp_path / "calls.log"
    for path in (bin_dir, tool_bin, codefa_home):
        path.mkdir(parents=True)
    (codefa_home / "config.json").write_text("{}", encoding="utf-8")
    for name in CODEFA_COMMANDS:
...
if [ "${1:-}" = "tool" ] && [ "${2:-}" = "uninstall" ]; then
    ...
    if [ "$FAIL_STEP" = "missing" ] || [ "$FAIL_STEP" = "stale-entrypoint" ]; then
        echo 'Tool `codefa` is not installed' >&2
        exit 2
    fi
    for name in codefa-server codefa-claude codefax codefa-pi codefa-init codefa; do
        /bin/rm -f "$FAKE_TOOL_BIN/$name"
    done
    echo "Uninstalled codefa"
    exit 0
fi
exit 43
""",
...
""",
    ...
    return PosixUninstallHarness(home, bin_dir, tool_bin, codefa_home, log, env)


def test_uninstall_sh_removes_and_verifies_only_codefa(
    ...
    assert not posix_uninstall_harness.codefa_home.exists()
    assert all(
    ...
    assert posix_uninstall_harness.calls() == [
        "uv:tool dir --bin",
        "uv:tool uninstall codefa",
        f"rm:-rf {posix_uninstall_harness.codefa_home}",
    ]
...
def test_uninstall_sh_preserves_config_when_tool_removal_is_unconfirmed(
    ...
    assert posix_uninstall_harness.codefa_home.exists()
...
def test_uninstall_sh_dry_run_is_non_mutating(
    ...
    assert posix_uninstall_harness.codefa_home.exists()
    assert all(
        (posix_uninstall_harness.tool_bin / name).exists() for name in CODEFA_COMMANDS
    )
    assert posix_uninstall_harness.calls() == []
...
def powershell_uninstall_harness(
    ...
    uv_commands = " ".join(CODEFA_COMMANDS)
...
if "%FAIL_STEP%"=="missing" echo Tool `codefa` is not installed 1>&2 & exit /b 2
if "%FAIL_STEP%"=="stale-entrypoint" echo Tool `codefa` is not installed 1>&2 & exit /b 2
...
$installer = [scriptblock]::Create([IO.File]::ReadAllText($env:CODEFA_UNINSTALLER))
    ...
    env.update(
        {
            ...
            "CALL_LOG": str(log),
            "FAKE_TOOL_BIN": str(tool_bin),
            "CODEFA_UNINSTALLER": str(_repo_root() / "scripts" / "uninstall.ps1"),
            "FAIL_STEP": "",
            "UNINSTALL_DRY_RUN": "0",
        }
    )
    return PowerShellUninstallHarness(
        home, bin_dir, tool_bin, codefa_home, log, env, powershell, wrapper
    )


def test_uninstall_ps1_removes_and_verifies_only_codefa(
    ...
    assert result.returncode == 0, result.stderr
    assert "Free Claude Code has been removed and verified." in result.stdout
    assert not powershell_uninstall_harness.codefa_home.exists()
    assert all(
    ...
    assert powershell_uninstall_harness.calls() == [
        "uv:tool dir --bin",
        "uv:tool uninstall codefa",
        f"remove:{powershell_uninstall_harness.codefa_home}",
    ]
...
def test_uninstall_ps1_preserves_config_when_tool_removal_is_unconfirmed(
    ...
    assert result.returncode != 0
    assert powershell_uninstall_harness.codefa_home.exists()
    assert "Free Claude Code has been removed and verified." not in result.stdout
    assert not any(
...
def test_uninstallers_guard_running_commands_and_preserve_shared_owners() -> None:
    ...
    for text in (shell, powershell):
        for command in CODEFA_COMMANDS:
...
def test_readme_uninstall_uses_raw_urls_and_verification_contract() -> None:
    ...
    assert (
        ...
        'Foshati/codefa/main/scripts/uninstall.sh" | sh'
    ) in text
    assert (
        '& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/'
        'Foshati/codefa/main/scripts/uninstall.ps1")))'
    ) in text
    assert "verifies every CODEFA command is gone" in text
