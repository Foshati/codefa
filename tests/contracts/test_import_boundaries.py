...
_PACKAGE_ROOT = _REPO_ROOT / "src" / "codefa"
_PACKAGE_NAME = "codefa"
...
FACADE_ONLY_BOUNDARIES = {
    "codefa.core.openai_responses",
    "codefa.messaging.trees",
    "codefa.providers.openai_chat",
}

OPTIONAL_IMPORT_OWNERS = {
    "librosa": "codefa.messaging.transcription",
    "torch": "codefa.messaging.transcription",
    "transformers": "codefa.messaging.transcription",
    "riva": "codefa.providers.nvidia_nim.voice",
}
...
def test_legacy_first_party_import_detector_rejects_bare_owner_names() -> None:
    record = ImportRecord(
        importer="codefa.api.routes",
        imported="core.anthropic",
        path="codefa/api/routes.py",
        line=7,
        inside_function=False,
    )

    assert _legacy_first_party_import_offenders([record], {"api", "core"}) == [
        "codefa/api/routes.py:7: codefa.api.routes -> core.anthropic"
    ]
...
def test_core_does_not_import_provider_transport_sdks() -> None:
    ...
    offenders = [
        ...
        for record in _scan_imports(_PACKAGE_ROOT)
        if (
            record.importer == "codefa.core"
            or record.importer.startswith("codefa.core.")
        )
...
def test_runtime_imports_without_optional_voice_dependencies() -> None:
    ...
    script = "\n".join(
        (
            ...
            "import codefa.runtime.bootstrap",
            "import codefa.api.app",
        )
    )
...
def test_supported_messaging_facade_is_explicit() -> None:
    import codefa.messaging as facade
    from codefa.messaging.managed_protocols import (
        ManagedClaudeSessionManagerProtocol,
        ManagedClaudeSessionProtocol,
    )
    from codefa.messaging.models import IncomingMessage, MessageScope
    from codefa.messaging.platforms.ports import OutboundMessenger
...
def test_message_tree_mutability_stays_behind_its_facade() -> None:
    import codefa.messaging.trees as facade
