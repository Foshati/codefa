...
from codefa.messaging.command_context import ReplyClearResult, StopOutcome
from codefa.messaging.models import MessageScope
from codefa.messaging.platforms.ports import MessagingStartupNotice
from codefa.messaging.session import SessionStore
from codefa.messaging.trees import (
    CancellationReason,
    CancellationResult,
    CancellationUiOwner,
    FailureResult,
...
from codefa.messaging.trees.transitions import CancellationEffect
...
async def test_handle_message_turn_trace_always_includes_full_message_text(
    ...
    with (
        ...
        patch("codefa.messaging.workflow.trace_event") as trace_mock,
    ):
...
async def test_terminal_close_waits_past_interactive_drain_timeout(
    ...
    monkeypatch.setattr(
        "codefa.messaging.trees.manager.CANCEL_TASK_DRAIN_TIMEOUT_S",
        0.01,
    )
...
async def test_session_info_rejects_unregistered_real_session_without_recording_it(
    ...
    from codefa.messaging.node_event_pipeline import (
