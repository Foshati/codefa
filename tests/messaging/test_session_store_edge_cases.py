...
from codefa.messaging.session.persistence import DebouncedJsonPersistence
from codefa.messaging.trees import TreeIdentity, TreeSnapshot
...
class TestSessionStoreSaveEdgeCases:
    ...
    def test_explicit_flush_reports_io_error_and_keeps_store_dirty(self, tmp_store):
        ...
        with (
            patch(
                "codefa.messaging.session.persistence.os.replace",
...
class TestSessionStoreTreeSnapshots:
    def test_unscoped_tree_without_legacy_ingress_is_reported_and_skipped(
        ...
        with patch("codefa.messaging.trees.snapshot.logger.warning") as warning:
