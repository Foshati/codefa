...
def _run_init(tmp_home: Path) -> tuple[str, Path]:
    ...
    from codefa.cli.commands import init

    env_file = tmp_home / ".codefa" / ".env"
    printed: list[str] = []
...
def test_init_migrates_home_checkout_env_before_template(tmp_path: Path) -> None:
    """init() preserves users who kept config in ~/codefa/.env."""
    legacy_env = tmp_path / "codefa" / ".env"
...
def test_init_migrates_legacy_xdg_env_before_template(tmp_path: Path) -> None:
    """init() preserves users who kept config in ~/.config/codefa/.env."""
    legacy_env = tmp_path / ".config" / "codefa" / ".env"
    legacy_env.parent.mkdir(parents=True)
...
def test_legacy_env_migration_does_not_overwrite_managed_env(
    ...
    """Legacy migration never overwrites an existing ~/.codefa/.env."""
    from codefa.cli.commands import _migrate_legacy_env_if_missing

    managed_env = tmp_path / ".codefa" / ".env"
...
def test_env_template_loader_uses_root_template_in_source_checkout() -> None:
    ...
    from codefa.config.env_template import load_env_template
...
def test_init_creates_parent_directories(tmp_path: Path) -> None:
    """init() creates ~/.codefa/ even if it doesn't exist."""
    config_dir = tmp_path / ".codefa"
...
def test_init_prints_next_step_hint(tmp_path: Path) -> None:
    """init() tells the user to run codefa-server after editing .env."""
    output, _ = _run_init(tmp_path)

    assert "codefa-server" in output
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert scripts["codefa-server"] == "codefa.cli.entrypoints:serve"
    assert scripts["codefa"] == "codefa.cli.entrypoints:combined"
    assert scripts["codefa-claude"] == "codefa.cli.launchers.claude:launch"
    assert scripts["codefax"] == "codefa.cli.launchers.codex:launch"
    assert scripts["codefa-pi"] == "codefa.cli.launchers.pi:launch"


@pytest.mark.parametrize("entrypoint_name", ["serve", "init", "combined"])
@pytest.mark.parametrize(
    "argv",
    [("--version",), ("--version", "--help"), ("--help", "--version")],
)
def test_codefa_owned_entrypoints_report_version_without_side_effects(
    entrypoint_name: str,
    argv: tuple[str, ...],
    capsys: pytest.CaptureFixture[str],
) -> None:
    from codefa.cli import entrypoints

    with patch.object(entrypoints, "package_version", return_value="9.8.7"):
        getattr(entrypoints, entrypoint_name)(argv)

    assert capsys.readouterr() == ("codefa 9.8.7\n", "")
...
def test_version_entrypoints_do_not_import_command_runtime(
    ...
    script = "\n".join(
        (
            "import json",
            "import sys",
            f"from codefa.cli.entrypoints import {entrypoint_name}",
            f"{entrypoint_name}(['--version'])",
            "forbidden = ('uvicorn', 'fastapi', 'openai', "
            "'codefa.cli.commands', "
            "'codefa.runtime.bootstrap')",
...
def test_non_version_entrypoints_delegate_to_command_implementation(
    ...
    from codefa.cli import commands, entrypoints
...
def test_schedule_open_admin_browser_opens_when_health_ready() -> None:
    ...
    from codefa.cli import commands
    from codefa.config.server_urls import local_admin_url
...
def test_serve_migrates_legacy_env_before_loading_settings(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".codefa" / ".env").read_text("utf-8") == (
...
def test_config_env_key_migration_warns_for_explicit_env_file(
    ...
    with patch.dict(commands.os.environ, {"CODEFA_ENV_FILE": str(explicit)}):
...
def test_claude_child_env_targets_current_proxy_config() -> None:
    from codefa.cli.claude_env import build_claude_proxy_env
...
def test_claude_child_env_uses_sentinel_for_blank_configured_auth_token() -> None:
    ...
    assert env["ANTHROPIC_AUTH_TOKEN"] == "codefa-no-auth"
    assert "ANTHROPIC_API_KEY" not in env
...
def test_launch_claude_passes_args_and_child_env(
    ...
    from codefa.cli.launchers.claude import launch
    ...
    with (
        patch("codefa.cli.launchers.claude.get_settings", return_value=settings),
        patch("codefa.cli.launchers.claude.preflight_proxy", return_value=None),
        patch(
            "codefa.cli.launchers.common.shutil.which",
            return_value="resolved-claude.cmd",
        ),
        patch("codefa.cli.launchers.common.subprocess.Popen") as popen,
        patch("codefa.cli.launchers.common.register_pid") as register_pid,
        patch("codefa.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    from codefa.cli.launchers.codex import launch
    ...
    with (
        patch("codefa.cli.launchers.codex.get_settings", return_value=settings),
        patch("codefa.cli.launchers.codex.preflight_proxy", return_value=None),
        patch(
            "codefa.cli.launchers.common.shutil.which",
            return_value="resolved-codex.cmd",
        ),
        patch(
            "codefa.cli.launchers.codex.codex_model_catalog_path",
            return_value=catalog_path,
        ),
        patch(
            "codefa.cli.launchers.codex.open_local_request",
    ...
    assert 'model_provider="codefa"' in command
    assert 'model_providers.codefa.base_url="http://127.0.0.1:9191/v1"' in command
    assert 'model_providers.codefa.wire_api="responses"' in command
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["CODEFAX_API_KEY"] == "proxy-token"
...
def test_launch_codex_catalog_failure_warns_and_continues(
    ...
    with (
        ...
        patch("codefa.cli.launchers.common.subprocess.Popen") as popen,
        patch("codefa.cli.launchers.common.register_pid"),
        patch("codefa.cli.launchers.common.unregister_pid"),
        pytest.raises(SystemExit) as exc_info,
    ):
        process = popen.return_value
        process.pid = 12345
...
def test_pi_launcher_builds_scoped_session_command_and_proxy_env(
    ...
    from codefa.cli.launchers.pi import (
    ...
    env = build_pi_launcher_env(
        ...
        base_env={
            ...
            "CODEFA_PI_API_KEY": "stale-key",
            "CODEFA_PI_BASE_URL": "https://stale.invalid",
        },
    )
    ...
    assert env == {
        ...
        "no_proxy": "127.0.0.1,localhost,::1",
        "CODEFA_PI_BASE_URL": "http://127.0.0.1:9191",
        "CODEFA_PI_API_KEY": "proxy-token",
    }


def test_pi_launcher_uses_no_auth_sentinel_for_blank_token() -> None:
    from codefa.cli.launchers.pi import build_pi_launcher_env
    ...
    assert env["CODEFA_PI_API_KEY"] == "codefa-no-auth"
...
def test_launch_pi_registers_bundled_extension_for_sessions(
    ...
    from codefa.cli.launchers.pi import launch

    monkeypatch.setenv("KEEP_ME", "yes")
    monkeypatch.setenv("CODEFA_PI_API_KEY", "stale-key")
    extension = tmp_path / "pi_extension.ts"
    extension.write_text("export default () => {};", encoding="utf-8")
    settings = _launcher_settings(port=9191, token="proxy-token")

    with (
        patch("codefa.cli.launchers.pi.get_settings", return_value=settings),
        patch("codefa.cli.launchers.pi.preflight_proxy", return_value=None),
        patch(
            "codefa.cli.launchers.pi.pi_extension_path",
        ...
        patch(
            "codefa.cli.launchers.pi.pi_binary_is_compatible",
            return_value=True,
        ),
        patch("codefa.cli.launchers.common.subprocess.Popen") as popen,
        patch("codefa.cli.launchers.common.register_pid"),
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["CODEFA_PI_BASE_URL"] == "http://127.0.0.1:9191"
    assert child_env["CODEFA_PI_API_KEY"] == "proxy-token"
    assert child_env["NO_PROXY"] == "127.0.0.1,localhost,::1"
    assert child_env["no_proxy"] == child_env["NO_PROXY"]
    assert child_env["KEEP_ME"] == "yes"
...
def test_launch_pi_passes_management_commands_through_without_proxy(
    ...
    with (
        patch("codefa.cli.launchers.pi.get_settings") as get_settings,
        patch("codefa.cli.launchers.pi.preflight_proxy") as preflight,
        patch(
            "codefa.cli.launchers.common.shutil.which",
...
def test_pi_install_hints_use_official_platform_installers() -> None:
    from codefa.cli.launchers.pi import pi_install_hint
...
def test_pi_binary_compatibility_requires_both_launcher_capabilities(
    ...
    from codefa.cli.launchers.pi import pi_binary_is_compatible

    with patch(
        "codefa.cli.launchers.pi.subprocess.run",
...
def test_launch_claude_keyboard_interrupt_kills_child_tree() -> None:
    ...
    with (
        ...
        patch("codefa.cli.launchers.common.subprocess.Popen") as popen,
        patch("codefa.cli.launchers.common.register_pid"),
        patch("codefa.cli.launchers.common.kill_pid_tree_best_effort") as kill_tree,
        patch("codefa.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_claude_exits_when_command_cannot_be_resolved(
    ...
    with (
        patch("codefa.cli.launchers.claude.get_settings", return_value=settings),
        patch("codefa.cli.launchers.claude.preflight_proxy", return_value=None),
        patch("codefa.cli.launchers.common.shutil.which", return_value=None),
        patch("codefa.cli.launchers.common.subprocess.Popen") as popen,
        pytest.raises(SystemExit) as exc_info,
    ):
        launch([])
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    with (
        patch("codefa.cli.launchers.claude.get_settings", return_value=settings),
        patch(
            "codefa.cli.launchers.claude.preflight_proxy",
    ...
    assert "codefa-server" in captured.err
