"""Local CODEFA transport must never escape through an outbound proxy."""
...
def test_proxy_preflight_connects_directly_when_http_proxy_is_configured(
    ...
    with _status_server(502) as (forward_proxy_url, forward_proxy_hits):
        ...
        with _status_server(200) as (codefa_url, codefa_hits):
            assert preflight_proxy(codefa_url) is None

    assert codefa_hits == ["/health"]
...
def test_child_proxy_bypass_preserves_existing_proxy_policy() -> None:
    ...
    env = with_local_proxy_bypass(
        base_env,
        proxy_root_url="http://codefa.internal:8082",
    )

    assert env["HTTP_PROXY"] == "http://proxy.example:3128"
    assert env["KEEP_ME"] == "yes"
    assert env["NO_PROXY"] == (
        "example.com,localhost,10.0.0.0/8,127.0.0.1,::1,codefa.internal"
    )
