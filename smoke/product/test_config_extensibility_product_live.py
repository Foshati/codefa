...
from codefa.providers.runtime import build_provider_config
...
def test_env_precedence_e2e(smoke_config: SmokeConfig, tmp_path) -> None:
    ...
    env = os.environ.copy()
    env["CODEFA_ENV_FILE"] = str(env_file)
...
def test_removed_env_migration_e2e(smoke_config: SmokeConfig, tmp_path) -> None:
    ...
    env["CODEFA_ENV_FILE"] = str(env_file)
    result = run_captured_text(
        cmd_python_c("from codefa.config.settings import Settings; Settings()"),
        cwd=smoke_config.root,
        env=env,
        timeout=smoke_config.timeout_s,
        check=False,
