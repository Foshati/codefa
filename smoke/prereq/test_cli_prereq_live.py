...
def test_codefa_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".codefa" / ".env").is_file()


def test_codefa_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = run_captured_text(
            [claude_bin, "-p", "Reply with exactly CODEFA_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            timeout=smoke_config.timeout_s,
            check=False,
    ...
    if "CODEFA_SMOKE_PONG" not in result.stdout:
