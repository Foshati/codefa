...
def _has_proxy_request(log_delta: str) -> bool:
    return (
        ...
        or '"event": "codefa.api.request.received"' in log_delta
        or (
...
def _request_count(log_delta: str) -> int:
    ...
    structured_log_count = log_delta.count('"event": "codefa.api.request.received"')
    return max(access_log_count, service_log_count, structured_log_count)


def _marker(scope: str, prefix: str) -> str:
    return f"CODEFA_{scope}_{prefix}_{uuid.uuid4().hex[:8].upper()}"
