...
(``codefa.exe`` in use), causing flaky smoke. The smoke runner is
...
def cmd_codefa_init() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from codefa.cli.entrypoints import init; init()",
    ]


def cmd_codefa_version() -> list[str]:
    return [
        python_exe(),
        "-c",
        (
            "import sys; "
            "sys.argv = ['codefa-server', '--version']; "
            "from codefa.cli.entrypoints import serve; serve()"
        ),
    ]


def cmd_codefa_serve() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from codefa.cli.entrypoints import serve; serve()",
