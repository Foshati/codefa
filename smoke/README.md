...
The second command skips everything unless `CODEFA_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `CODEFA_SMOKE_MODEL_<PROVIDER>`, for example
`CODEFA_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`CODEFA_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `codefa-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `CODEFA_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `CODEFA_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "ollama"
...
```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "telegram,discord,voice"
$env:CODEFA_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "nvidia_nim_cli"
$env:CODEFA_SMOKE_NIM_MODELS = "z-ai/glm-5.2,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "openrouter_free_cli"
$env:CODEFA_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEFA_LIVE_SMOKE = "1"
$env:CODEFA_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `CODEFA_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `CODEFA_LIVE_SMOKE=1`: enables live smoke execution.
- `CODEFA_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `CODEFA_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `CODEFA_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `CODEFA_SMOKE_MODEL_NVIDIA_NIM`, `CODEFA_SMOKE_MODEL_OPEN_ROUTER`,
  `CODEFA_SMOKE_MODEL_MISTRAL`, `CODEFA_SMOKE_MODEL_MISTRAL_REASONING`,
  `CODEFA_SMOKE_MODEL_MISTRAL_CODESTRAL`,
  `CODEFA_SMOKE_MODEL_DEEPSEEK`, `CODEFA_SMOKE_MODEL_KIMI`,
  `CODEFA_SMOKE_MODEL_KIMI_CODE`,
  `CODEFA_SMOKE_MODEL_WAFER`, `CODEFA_SMOKE_MODEL_MINIMAX`,
  `CODEFA_SMOKE_MODEL_OPENCODE`, `CODEFA_SMOKE_MODEL_OPENCODE_GO`,
  `CODEFA_SMOKE_MODEL_BEDROCK`,
  `CODEFA_SMOKE_MODEL_ZAI`, `CODEFA_SMOKE_MODEL_FIREWORKS`, `CODEFA_SMOKE_MODEL_CLOUDFLARE`,
  `CODEFA_SMOKE_MODEL_GEMINI`, `CODEFA_SMOKE_MODEL_GROQ`, `CODEFA_SMOKE_MODEL_CEREBRAS`,
  `CODEFA_SMOKE_MODEL_OLLAMA_CLOUD`, `CODEFA_SMOKE_MODEL_LMSTUDIO`,
  `CODEFA_SMOKE_MODEL_LLAMACPP`, `CODEFA_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `CODEFA_SMOKE_MODEL_MISTRAL_REASONING`: optional override for the dedicated
  Mistral native reasoning smoke, default `mistral/mistral-medium-3-5`.
- `CODEFA_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `CODEFA_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `CODEFA_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `CODEFA_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `CODEFA_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `CODEFA_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `CODEFA_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `CODEFA_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `CODEFA_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `CODEFA_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
`uv run`, so Windows does not try to replace `codefa.exe` while it is
locked.
...
- `target_disabled`: skipped because `CODEFA_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `CODEFA_SMOKE_PROVIDER_MATRIX`;
