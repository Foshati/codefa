...
from codefa.core.anthropic import anthropic_error_type_for_failure
...
from codefa.core.async_iterators import try_close_async_iterator
from codefa.core.diagnostics import safe_exception_message
from codefa.core.failures import find_execution_failure
from codefa.core.trace import close_stream_input, trace_event
    ...
    async def _close(self, *, preserved_error: BaseException | None) -> None:
        ...
        if task is None:
            task = asyncio.create_task(
                ...
                name="codefa-api-response-cleanup",
            )
...
def _trace_response_cleanup_failure(operation: str, exc: BaseException) -> None:
    trace_event(
        stage="egress",
        event="codefa.api.response.cleanup_failed",
        source="api",
        operation=operation,
...
def trace_terminal_execution_error(
    *,
    ...
    fields: dict[str, object] = {
        "stage": "egress",
        "event": "codefa.api.response.terminal_execution_error",
        "source": "api",
        "wire_api": wire_api,
...
async def _close_pre_start_body(
    ...
    *,
    ...
    task = asyncio.create_task(
        ...
        name="codefa-api-pre-start-stream-cleanup",
    )
    await _wait_for_cleanup(task)
