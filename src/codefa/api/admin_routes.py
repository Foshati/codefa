...
from codefa.config.admin.manifest import FIELD_BY_KEY
from codefa.config.admin.persistence import validate_updates
from codefa.config.admin.values import load_config_response
from codefa.config.model_refs import configured_chat_model_refs
...
async def admin_asset(filename: str, request: Request):
    require_loopback_admin(request)
    if filename not in {"admin.css", "admin.js", "foshati.jpg"}:
        raise HTTPException(status_code=404, detail="Admin asset not found")
