...
from codefa.api.detection import is_safety_classifier_request
from codefa.api.optimization_handlers import try_optimizations
from codefa.api.request_errors import (
    http_status_for_unexpected_api_exception,
    log_unexpected_api_exception,
    require_non_empty_messages,
    unexpected_http_exception,
)
from codefa.api.request_ids import new_request_id
from codefa.api.response_streams import (
    EmptyStreamError,
...
from codefa.api.web_tools.request import (
    is_web_server_tool_request,
    unsupported_server_tool_error,
)
from codefa.api.web_tools.streaming import stream_web_server_tool_response
from codefa.application.errors import ApplicationError, InvalidRequestError
from codefa.application.execution import ProviderExecutor, TokenCounter
from codefa.application.ports import ProviderResolver
from codefa.application.routing import ModelRouter, RoutedMessagesRequest
from codefa.config.settings import Settings
from codefa.core.anthropic import (
    MessagesRequest,
...
from codefa.core.diagnostics import safe_exception_message
from codefa.core.failures import ExecutionFailure, find_execution_failure
from codefa.core.reasoning import ReasoningControl, ReasoningPolicy
from codefa.core.trace import trace_event
...
class MessagesHandler:
    ...
    def _apply_message_routing_policies(
        ...
        trace_event(
            stage="routing",
            event="codefa.api.optimization.safety_classifier_no_thinking",
            source="api",
    ...
    def _intercept_web_server_tool(
        ...
        trace_event(
            stage="routing",
            event="codefa.api.optimization.web_server_tool",
            source="api",
            model=routed.request.model,
        )
    ...
    def _intercept_local_optimization(
        ...
        trace_event(
            stage="routing",
            event="codefa.api.optimization.short_circuit",
            source="api",
            model=routed.request.model,
        )
