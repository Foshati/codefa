...
from codefa.cli.process_registry import kill_all_best_effort
from codefa.config.env_migrations import (
    explicit_env_file_migration_warning,
    migrate_owned_env_files,
)
from codefa.config.env_template import load_env_template
from codefa.config.paths import (
    config_dir_path,
    legacy_env_paths,
    managed_env_path,
)
...
from codefa.runtime.bootstrap import build_asgi_app
...
def _schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        target=open_when_ready, name="codefa-open-admin-browser", daemon=True
    ).start()
...
def init() -> None:
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: codefa-server"
        )
        return
    ...
    print("Edit it to set your API keys and model preferences, then run: codefa-server")
...
def _migrate_legacy_env_if_missing() -> Path | None:
    ...
    # TODO: Remove after the ~/.codefa/.env migration has had a release cycle.
