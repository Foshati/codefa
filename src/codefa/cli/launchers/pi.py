...
from codefa.cli.local_http import with_local_proxy_bypass
...
_API_KEY_ENV = "CODEFA_PI_API_KEY"
_BASE_URL_ENV = "CODEFA_PI_BASE_URL"
_BINARY_NAME = "pi"
_DISPLAY_NAME = "Pi"
...
def launch(argv: Sequence[str] | None = None) -> None:
    ...
    if not extension_path.is_file():
        print(
            "Free Claude Code's bundled Pi extension is missing. Reinstall CODEFA.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def build_pi_launcher_command(
    *,
    ...
    """Return a Pi session command with ephemeral CODEFA provider registration."""
...
def build_pi_launcher_env(
    *,
    ...
    """Return a Pi environment containing only CODEFA-owned proxy variables."""

    env = with_local_proxy_bypass(
        {
            key: value
            for key, value in base_env.items()
            if not key.startswith("CODEFA_PI_")
        },
        proxy_root_url=proxy_root_url,
    )
...
def pi_binary_is_compatible(binary_path: str) -> bool:
    """Return whether an executable exposes the Pi features CODEFA requires."""
