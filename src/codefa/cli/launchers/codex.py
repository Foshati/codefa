...
from codefa.cli.local_http import (
    open_local_request,
    with_local_proxy_bypass,
)
from codefa.cli.proxy_auth import proxy_auth_token
from codefa.config.paths import codex_model_catalog_path
...
_CODEX_AUTH_ENV_KEY = "CODEFAX_API_KEY"
...
def launch(argv: Sequence[str] | None = None) -> None:
    ...
    if error := preflight_proxy(proxy_root_url):
        ...
        print("Start it in another terminal with: codefa-server", file=sys.stderr)
        raise SystemExit(1)
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral CODEFA provider config."""
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral CODEFA provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "codefa"),
        "-c",
        _toml_assignment("model_providers.codefa.name", "Free Claude Code"),
        "-c",
        _toml_assignment("model_providers.codefa.base_url", _ensure_v1_url(api_url)),
        "-c",
        _toml_assignment("model_providers.codefa.env_key", _CODEX_AUTH_ENV_KEY),
        "-c",
        _toml_assignment("model_providers.codefa.wire_api", "responses"),
    ]
    if model:
