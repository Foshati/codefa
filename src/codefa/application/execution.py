    ...
    def stream(
        ...
        *,
        ...
        trace_event(
            stage="ingress",
            event=(
                "codefa.api.responses.request.received"
                if wire_api == "responses"
                else "codefa.api.request.received"
            ),
            source="api",
        ...
        return traced_async_stream(
            ...
            source="api",
            complete_event=(
                "codefa.api.responses.stream_completed"
                if wire_api == "responses"
                else "codefa.api.response.stream_completed"
            ),
            interrupted_event=(
                "codefa.api.responses.stream_interrupted"
                if wire_api == "responses"
                else "codefa.api.response.stream_interrupted"
            ),
            chunk_event=None,
            extra=stream_trace,
        )
