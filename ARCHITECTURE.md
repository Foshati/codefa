...
- [src/codefa/application/](src/codefa/application/) is the dependency-leaf application boundary. It
...
- [src/codefa/api/](src/codefa/api/) is the HTTP adapter. It owns the FastAPI app, routes, API product
...
- [src/codefa/cli/](src/codefa/cli/) owns console entrypoints, client CLI launchers, process/session
  management, and client adapter contracts.
- [src/codefa/config/](src/codefa/config/) owns settings, provider metadata, filesystem paths,
  logging setup, constants, and provider ID catalogs.
- [src/codefa/core/](src/codefa/core/) owns provider-neutral protocol logic: wire request and response
...
- [src/codefa/messaging/](src/codefa/messaging/) owns optional platform adapters, incoming message
  handling, tree queues, transcript rendering, persistence, commands, and voice
  support.
- [src/codefa/providers/](src/codefa/providers/) owns provider construction, the shared OpenAI-chat
...
- [src/codefa/runtime/](src/codefa/runtime/) is the process composition root. It owns application
...
`codefa.cli.entrypoints` imports
`codefa.runtime.bootstrap` because the installed server executable
...
[core/version.py](src/codefa/core/version.py) is the sole runtime owner
of the CODEFA release version. It reads installed distribution metadata for
FastAPI/OpenAPI, CODEFA-owned CLI `--version` output, and the outbound web-tools
...
their wrapped clients except for CODEFA-owned ephemeral provider configuration.

The main ownership rule is that Anthropic and Responses protocol schemas and
shared protocol behavior belong in [src/codefa/core/](src/codefa/core/), while request routing and
provider execution belong in [src/codefa/application/](src/codefa/application/). Routes use core schemas
...
CODEFA optimizes for installed user workflows, not internal compatibility. The
...
- `codefa-server` and the local Admin UI for configuring supported providers,
  model routing, auth, server tools, messaging, and diagnostics.
- `codefa-claude`, Claude Code, and the Anthropic-compatible proxy behavior Claude
...
- `codefax`, Codex CLI/extensions, and the streaming OpenAI Responses behavior
...
- `codefa-pi`, Pi, and the Anthropic-compatible proxy behavior Pi relies on,
  including an CODEFA-scoped model catalog, streaming text and reasoning, and tool
  use/results.
...
- [api/handlers/](src/codefa/api/handlers/) owns customer-facing API product flows:
  ...
  [application/execution.py](src/codefa/application/execution.py) only for shared
  provider resolution, preflight, tracing, token counting, and streaming.
- [providers/openai_chat/](src/codefa/providers/openai_chat/) owns the common upstream provider
  ...
  protocol rules belong in [src/codefa/core/](src/codefa/core/).
- [messaging/workflow.py](src/codefa/messaging/workflow.py) coordinates messaging runtime
...
- [config/admin/](src/codefa/config/admin/) owns Admin UI config behavior. Keep
...
- `codefa-server` and `codefa` call `codefa.cli.entrypoints:serve`.
- `codefa-init` calls `codefa.cli.entrypoints:init`.
- `codefa-claude` calls `codefa.cli.launchers.claude:launch`.
- `codefax` calls `codefa.cli.launchers.codex:launch`.
- `codefa-pi` calls `codefa.cli.launchers.pi:launch`.
...
and [scripts/uninstall.ps1](scripts/uninstall.ps1) remove only the CODEFA uv tool and always
delete the managed `~/.codefa/` tree from [config/paths.py](src/codefa/config/paths.py); they do not remove
...
[cli/entrypoints.py](src/codefa/cli/entrypoints.py) starts the FastAPI server with Uvicorn.
...
[runtime/bootstrap.py](src/codefa/runtime/bootstrap.py) is the single production composition function. The CLI
...
configures logging, constructs the runtime owners and the configured voice
  ...
  the consumer-owned ports in [application/ports.py](src/codefa/application/ports.py); Admin operations retain
  their inbound-adapter port in [api/ports.py](src/codefa/api/ports.py).

[api/app.py](src/codefa/api/app.py) registers routers and exception
...
[runtime/application.py](src/codefa/runtime/application.py) owns process startup and shutdown, optional messaging,
...
[runtime/asgi.py](src/codefa/runtime/asgi.py) drives that owner from ASGI lifespan messages and preserves
the concise startup-failure contract.

[runtime/provider_manager.py](src/codefa/runtime/provider_manager.py) is the only owner that constructs, publishes,
...
streaming responses bind it to CODEFA's response owner, which first closes the
...
[config/settings.py](src/codefa/config/settings.py) owns the flat Pydantic Settings schema:
...
in [config/env_files.py](src/codefa/config/env_files.py) and uses this order:

1. repo-local `.env`;
2. managed `~/.codefa/.env`;
3. optional `CODEFA_ENV_FILE`, appended when present.
...
source detection for startup warnings also belongs to `src/codefa/config/env_files.py`.

[config/paths.py](src/codefa/config/paths.py) defines managed paths:

- config directory: `~/.codefa`;
- managed env file: `~/.codefa/.env`;
- generated Codex model catalog: `~/.codefa/codex-model-catalog.json`;
- messaging state directory: `~/.codefa/agent_workspace`;
- server log: `~/.codefa/logs/server.log`.
...
[config/reasoning.py](src/codefa/config/reasoning.py) owns the typed
configuration vocabulary. CODEFA-owned dotenv files receive a one-time rename and
value migration from the retired boolean settings; explicit `CODEFA_ENV_FILE`
files are never rewritten and instead receive an actionable startup warning.

[config/model_refs.py](src/codefa/config/model_refs.py) owns provider-prefixed model ref
...
[config/admin/](src/codefa/config/admin/) owns the Admin UI config manifest and
managed env writes. Provider credential, configurable base URL, proxy, and display-name
metadata is generated from [config/provider_catalog.py](src/codefa/config/provider_catalog.py);
...
separate modules. [api/admin_routes.py](src/codefa/api/admin_routes.py) exposes local-only
...
It is packaged as a [src/codefa/config/](src/codefa/config/) resource for `codefa-init` and Admin UI
...
[api/routes.py](src/codefa/api/routes.py) exposes the public proxy routes:
...
Admin routes live beside these in [api/admin_routes.py](src/codefa/api/admin_routes.py).

Authentication is handled by `require_proxy_auth()` in
[api/dependencies.py](src/codefa/api/dependencies.py). If `ANTHROPIC_AUTH_TOKEN` is blank,
proxy auth is disabled. Otherwise CODEFA accepts exactly `Authorization: Bearer
...
opaque CODEFA request ID before routing, places it in log context and request state,
...
[api/handlers/](src/codefa/api/handlers/) owns the public API product flows.
...
[application/execution.py](src/codefa/application/execution.py). `ProviderExecutor` resolves the narrow
...
object.
[api/response_streams.py](src/codefa/api/response_streams.py) owns public streaming egress
commit timing. It waits for the first protocol chunk before returning a
successful CODEFA-owned `StreamingResponse`. Its explicit replay iterator owns the
...
response. Once CODEFA has finalized the failure, the response includes
`x-should-retry: false` so CODEFA retains ownership of upstream retry/recovery
...
[src/codefa/core/openai_responses/adapter.py](src/codefa/core/openai_responses/adapter.py). The adapter
...
[application/routing.py](src/codefa/application/routing.py) resolves incoming client model names.
It supports two forms:

- Direct provider model refs such as `nvidia_nim/nvidia/model-name`.
- Gateway model IDs decoded by [core/gateway_model_ids.py](src/codefa/core/gateway_model_ids.py).
...
[application/reasoning.py](src/codefa/application/reasoning.py) then
...
Codex-specific model picker shaping stays out of this route. `codefax` fetches
the same `/v1/models` response at launch, converts CODEFA gateway IDs into
provider-selectable Codex slugs, writes `~/.codefa/codex-model-catalog.json`, and
passes it as `model_catalog_json`. Codex users open the native picker with
`/model`; CODEFA does not implement a proxy-level `/models` alias.

## Provider Architecture

Provider metadata is neutral and centralized in
[config/provider_catalog.py](src/codefa/config/provider_catalog.py). Each
...
[providers/runtime/](src/codefa/providers/runtime/) owns construction details for one
...
cleanup. [providers/runtime/factory.py](src/codefa/providers/runtime/factory.py)
...
[providers/admission.py](src/codefa/providers/admission.py) owns the
...
[application/model_metadata.py](src/codefa/application/model_metadata.py) owns the immutable
...
[application/ports.py](src/codefa/application/ports.py) defines the two provider operations consumed by request
...
[providers/base.py](src/codefa/providers/base.py) defines provider-internal construction and lifecycle contracts:
...
[providers/openai_chat/](src/codefa/providers/openai_chat/) implements the concrete
...
[providers/google_openai/](src/codefa/providers/google_openai/) owns the
...
combining it with CODEFA reasoning controls fails during deterministic preflight.
...
API-key endpoint; [providers/vertex/](src/codefa/providers/vertex/)
...
to Kimi's named efforts and identifies CODEFA through the upstream user agent.
...
[providers/mistral/reasoning.py](src/codefa/providers/mistral/reasoning.py), including its
...
downgrade: if an upstream NIM deployment rejects explicit budget control, CODEFA
retries without the budget while preserving thinking enablement.

### Reasoning Ownership

[core/reasoning.py](src/codefa/core/reasoning.py) owns the immutable,
...
effort through CODEFA's single product scale: `minimal`/`low=512`, `medium=1024`,
...
2. Prefer a provider's named effort vocabulary; use CODEFA's documented numeric
...
3. Never use the output-token limit as a reasoning budget. Forward exact or
   CODEFA-mapped budgets only through documented numeric fields; otherwise translate
...
1. Add provider metadata to [config/provider_catalog.py](src/codefa/config/provider_catalog.py).
2. Add credentials and related settings to [config/settings.py](src/codefa/config/settings.py)
...
3. Let Admin UI provider credential, configurable base URL, and proxy fields come from the
   ...
   [config/admin/](src/codefa/config/admin/) only when the generated manifest is
   insufficient.
4. Add an `OpenAIChatProfile` under [providers/openai_chat/](src/codefa/providers/openai_chat/) when
...
[src/codefa/core/anthropic/](src/codefa/core/anthropic/) owns Anthropic-side protocol behavior:
...
- stream lifecycle through `src/codefa/core/anthropic/streaming`, including the neutral
...
[src/codefa/core/anthropic/streaming/](src/codefa/core/anthropic/streaming/). The shared layer owns the
...
[core/failures.py](src/codefa/core/failures.py) defines the immutable,
...
[core/diagnostics.py](src/codefa/core/diagnostics.py) owns bounded error
...
[providers/failure_policy.py](src/codefa/providers/failure_policy.py)
...
[providers/stream_recovery.py](src/codefa/providers/stream_recovery.py)
...
execution budget. The OpenAI SDK's internal retries remain disabled so CODEFA is
the only retry owner. `ExecutionFailure.retryable` records provider-policy
eligibility; it never tells the client to retry after CODEFA has finalized the
failure.
...
[src/codefa/core/openai_responses/](src/codefa/core/openai_responses/) owns OpenAI Responses support:
...
- streaming-only `/v1/responses` support for Codex/CODEFA workflows;
...
CODEFA accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed CODEFA/Codex workflows only need
...
entrypoint;
[src/codefa/core/openai_responses/streaming/](src/codefa/core/openai_responses/streaming/) owns the
...
CODEFA does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and CODEFA
...
[api/optimization_handlers.py](src/codefa/api/optimization_handlers.py) short-circuits
...
[api/web_tools/](src/codefa/api/web_tools/). When `ENABLE_WEB_SERVER_TOOLS` is true, the
Messages handler can stream local Anthropic server-tool responses without sending the
request upstream. [api/web_tools/egress.py](src/codefa/api/web_tools/egress.py) enforces URL
...
[cli/local_http.py](src/codefa/cli/local_http.py) owns the direct
agent-to-CODEFA connection boundary. Launcher health checks and local model-catalog
...
adds the configured CODEFA host and standard loopback names to both `NO_PROXY` and
`no_proxy`. Provider-specific upstream proxies remain provider-owned and do not
participate in this local boundary.

[cli/proxy_auth.py](src/codefa/cli/proxy_auth.py) owns the neutral
proxy-auth token policy shared by client launchers. A blank configured token
becomes the local-only `codefa-no-auth` sentinel so clients cross their login gates
while CODEFA continues to run without API authentication.

[cli/claude_env.py](src/codefa/cli/claude_env.py) owns the canonical
Claude Code proxy environment used by every CODEFA-launched Claude process. It
...
[cli/launchers/claude.py](src/codefa/cli/launchers/claude.py) owns the installed
`codefa-claude` launcher:

- `codefa-claude` applies the shared proxy environment without changing the user's
  Claude command arguments.

[cli/launchers/codex.py](src/codefa/cli/launchers/codex.py) owns the installed
`codefax` launcher:

- `codefax` strips official OpenAI and Codex credential variables.
...
- It creates an ephemeral `codefa` model provider with `wire_api = "responses"` and
  a base URL pointing at the local proxy `/v1` path.
- After proxy health succeeds, it fetches `/v1/models`, writes a generated Codex
  `model_catalog_json` file under `~/.codefa/`, and injects that path so Codex's
  native `/model` picker lists CODEFA provider slugs. Catalog generation is
...
- It stores the proxy auth token in `CODEFAX_API_KEY` for Codex's provider
  `env_key` to read. This process-local variable is a client credential carrier,
  not a second CODEFA setting.

[cli/launchers/pi.py](src/codefa/cli/launchers/pi.py) owns the installed
`codefa-pi` launcher and [cli/launchers/pi_extension.ts](src/codefa/cli/launchers/pi_extension.ts)
is its bundled Pi adapter:

- Session commands load the extension from its absolute installed path and
  scope Pi to the ephemeral `codefa/**` provider, whose model IDs
  retain CODEFA's nested `provider/model` routing reference.
- The extension fetches CODEFA's `/v1/models` catalog before registration, projects
...
- CODEFA connection values live only in child-process `CODEFA_PI_*` variables. Native
...
- Pi package-management, configuration, help, and version commands pass through
  unchanged because they do not create an CODEFA-backed session.

[cli/managed/](src/codefa/cli/managed/) owns managed Claude Code subprocesses used by
...
Codex and Pi are supported through their installed launchers. CODEFA does not keep
...
Messaging is optional. [runtime/application.py](src/codefa/runtime/application.py) calls
`create_messaging_components()` from
[messaging/platforms/factory.py](src/codefa/messaging/platforms/factory.py) during startup.
...
[messaging/platforms/ports.py](src/codefa/messaging/platforms/ports.py): a
...
Runtime adapters in
[messaging/platforms/telegram.py](src/codefa/messaging/platforms/telegram.py) and
[messaging/platforms/discord.py](src/codefa/messaging/platforms/discord.py) own SDK client
...
[messaging/platforms/telegram_inbound.py](src/codefa/messaging/platforms/telegram_inbound.py)
and [messaging/platforms/discord_inbound.py](src/codefa/messaging/platforms/discord_inbound.py).
Outbound SDK calls live in
[messaging/platforms/telegram_io.py](src/codefa/messaging/platforms/telegram_io.py) and
[messaging/platforms/discord_io.py](src/codefa/messaging/platforms/discord_io.py). Shared
delivery policy lives in [messaging/platforms/outbox.py](src/codefa/messaging/platforms/outbox.py),
...
[messaging/platforms/voice_flow.py](src/codefa/messaging/platforms/voice_flow.py), which owns
...
in the `PendingVoiceRegistry` owned by [messaging/voice.py](src/codefa/messaging/voice.py).
...
[messaging/workflow.py](src/codefa/messaging/workflow.py) contains `MessagingWorkflow`, the
...
[messaging/turn_intake.py](src/codefa/messaging/turn_intake.py) owns slash command dispatch,
...
[messaging/node_runner.py](src/codefa/messaging/node_runner.py) owns managed CLI session
...
[messaging/event_parser.py](src/codefa/messaging/event_parser.py) normalizes managed Claude
JSON events into low-level transcript events.
[messaging/transcript/](src/codefa/messaging/transcript/) owns transcript assembly and
...
[messaging/rendering/](src/codefa/messaging/rendering/).

[messaging/command_context.py](src/codefa/messaging/command_context.py) defines the typed
...
[messaging/trees/runtime.py](src/codefa/messaging/trees/runtime.py) contains the
...
`parent_reference_id` records the exact prompt or CODEFA status that received the
...
[messaging/trees/transitions.py](src/codefa/messaging/trees/transitions.py) owns frozen,
...
[messaging/trees/manager.py](src/codefa/messaging/trees/manager.py) is the only external
...
[messaging/trees/repository.py](src/codefa/messaging/trees/repository.py)
is manager-private and owns only aggregate/reference indexes.
[messaging/trees/processor.py](src/codefa/messaging/trees/processor.py) owns every
...
[messaging/trees/node.py](src/codefa/messaging/trees/node.py) owns
...
[messaging/trees/graph.py](src/codefa/messaging/trees/graph.py) owns parent/child and
status-message lookup state, and
[messaging/trees/snapshot.py](src/codefa/messaging/trees/snapshot.py) owns typed persisted
...
[messaging/session/](src/codefa/messaging/session/) persists typed conversation snapshots
...
[messaging/session/persistence.py](src/codefa/messaging/session/persistence.py). One writer
...
ownership lives in
[messaging/session/managed_message_log.py](src/codefa/messaging/session/managed_message_log.py).
The registry accepts managed inbound prompts, voice notes, and commands as well
as CODEFA output. It migrates legacy `message_log` entries and persists the final
...
`/clear` commits CODEFA state cleanup first and then best-effort deletes the exact
authorized message-ID set through the list-based outbound port. Standalone clear
deletes every tracked user and CODEFA message in its chat; reply clear deletes only
...
missing messages; such failures never restore cleared CODEFA state.
...
[core/trace.py](src/codefa/core/trace.py) emits structured trace events across stages such
...
1. Add or expose the setting in [config/settings.py](src/codefa/config/settings.py).
2. Add the template key to [.env.example](.env.example) if users configure it.
3. Add a `ConfigFieldSpec` under [config/admin/](src/codefa/config/admin/), or add
...
1. For an installed wrapper, add or update a launcher under
   [cli/launchers/](src/codefa/cli/launchers/) and keep credential stripping local to that
   client.
2. For messaging-managed execution, update [cli/managed/](src/codefa/cli/managed/) only
...
3. Ensure managed task parsing emits the event shapes expected by
   [messaging/event_parser.py](src/codefa/messaging/event_parser.py) and
   [messaging/node_event_pipeline.py](src/codefa/messaging/node_event_pipeline.py).
...
1. Implement a `MessagingRuntime`, `OutboundMessenger`, and inbound normalizer
   under [messaging/platforms/](src/codefa/messaging/platforms/).
2. Reuse [messaging/platforms/outbox.py](src/codefa/messaging/platforms/outbox.py) for
   queued outbound delivery and
   [messaging/platforms/voice_flow.py](src/codefa/messaging/platforms/voice_flow.py) for
   voice-note handoff when the platform supports audio.
3. Add construction logic to
   [messaging/platforms/factory.py](src/codefa/messaging/platforms/factory.py).
4. Add settings and admin fields for tokens, allowlists, and platform-specific
   runtime options.
5. Add rendering profile support in
   [messaging/rendering/profiles.py](src/codefa/messaging/rendering/profiles.py) if needed.
6. Add deterministic runtime/outbound/workflow tests and optional live smoke
   targets.

### Add Protocol Behavior

1. Put shared Anthropic behavior under [src/codefa/core/anthropic/](src/codefa/core/anthropic/).
2. Put OpenAI Responses behavior under
   [src/codefa/core/openai_responses/](src/codefa/core/openai_responses/).
