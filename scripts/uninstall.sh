...
is_codefa_command_running() {
    [ -n "$(codefa_process_ids "$1")" ]
}

assert_no_codefa_processes_running() {
    running=""
    for command_name in $CODEFA_COMMANDS; do
        if is_codefa_command_running "$command_name"; then
...
initialize_uv_context() {
    ...
    if ! command -v uv >/dev/null 2>&1; then
        fail "uv is required to remove the Free Claude Code tool. Install uv, then rerun this uninstaller; ~/.codefa was not deleted."
    fi
    ...
    if uv_tool_bin=$(uv tool dir --bin); then
        ...
        status=$?
        fail "Could not determine the uv tool bin directory (exit code $status); ~/.codefa was not deleted."
    fi
    [ -n "$uv_tool_bin" ] || fail "uv returned an empty tool bin directory; ~/.codefa was not deleted."
}
...
uninstall_codefa() {
    ...
    fail "uv tool uninstall $PACKAGE_NAME failed with exit code $status; ~/.codefa was not deleted."
}

verify_codefa_commands_removed() {
    if [ "$dry_run" -eq 1 ]; then
    ...
    if [ -n "$remaining" ]; then
        fail "Free Claude Code entry points remain after uv uninstall:${remaining}; ~/.codefa was not deleted."
    fi
}

purge_codefa_home() {
    codefa_home="$HOME/$CODEFA_HOME_DIRNAME"
    if [ ! -e "$codefa_home" ]; then
        printf 'No CODEFA config directory at %s; skipping purge.\n' "$codefa_home"
        return 0
    fi

    run rm -rf "$codefa_home"
    if [ "$dry_run" -eq 0 ] && [ -e "$codefa_home" ]; then
        fail "CODEFA config directory still exists after deletion: $codefa_home"
    fi
}
...
step "Purging CODEFA config and data from ~/.codefa"
purge_codefa_home
