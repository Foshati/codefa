...
REPO_ARCHIVE_URL="https://github.com/Foshati/codefa/archive/refs/heads/main.zip"
...
assert_no_codefa_processes_running() {
    ...
    for command_name in $CODEFA_COMMANDS; do
        process_ids=$(codefa_process_ids "$command_name")
...
package_spec() {
    ...
    if [ "$include_nim" -eq 1 ] && [ "$include_local" -eq 1 ]; then
        printf 'codefa[voice,voice_local] @ %s' "$REPO_ARCHIVE_URL"
    elif [ "$include_nim" -eq 1 ]; then
        printf 'codefa[voice] @ %s' "$REPO_ARCHIVE_URL"
    elif [ "$include_local" -eq 1 ]; then
        printf 'codefa[voice_local] @ %s' "$REPO_ARCHIVE_URL"
    else
        printf 'codefa @ %s' "$REPO_ARCHIVE_URL"
    fi
}
