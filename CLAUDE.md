...
- Repository protection should use rulesets: a non-bypassable main integrity ruleset requires pull requests, merge queue, required checks, and blocks direct/force pushes to `main`; a separate review ruleset may allow `Foshati`/admins to bypass review only.
...
- **Shared utilities**: Put shared Anthropic protocol logic in neutral `src/codefa/core/anthropic/` modules. Do not have one provider import from another provider's utils.
...
- `src/codefa/api/`, `src/codefa/cli/`, `src/codefa/config/`, `src/codefa/core/`, `src/codefa/messaging/`, `src/codefa/providers/`
- `src/codefa/application/`
- `.env.example`
